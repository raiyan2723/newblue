...
class Login_model extends CI_Model
{
  ...
  //echo "<pre>"; echo $this->db->last_query();exit("here");
    ...
    function change_password($user, $pass)
    {
        ...
        $res = $this->db->update('test_user', $data);
        if ($res) {
            return "success";
        } else {
            return "failed";
    ...
    {
        $sql  = $this->db->query("select * from login where u_id='$user_id'");
        $data = $sql->result_array();
        return $data;
    }
    function update_signup($password, $u_id)
    {
        $sql = $this->db->query("UPDATE login SET password='$password' WHERE user_id='$user_id'");
    }
}
?>